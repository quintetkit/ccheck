...
| 権限ルール（続き） | パスの `Read` / `Edit` deny と、自分でファイルを開けるインタプリタの allow が同居している |
...
スコープ付きのキーは `data/settings-scopes.json` にあります。
**検査ツールが読むのと同じスナップショットから生成**しています。
...
**この道具を信用しなくても、主張を検算できる形**にしてあります。
公式の表とご自分で突き合わせてください。

別のものを作っていて**表を手で写したくない**場合も、これを読んでください。
検査ツールが71個のうち20個しか持っていなかったのは、**手で写したから**です。
...
**「正しく書けているのに何も起きない」がひとつの型だと気づいていない**なら、
まずこちらから:
**[正しく書いたのに何も起きない設定の全一覧](https://quintetkit.github.io/reference/claude-code-silent-config.html)**。
